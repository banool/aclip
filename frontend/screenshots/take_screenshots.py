"""
This whole script is a bit janky, sometimes all you need is some strategic retries and
flutter cleans and what was previously not working will magically start to work.
"""
...
# Taken from inspecting `flutter emulators`
ANDROID_EMULATORS = [
    "Nexus_7_API_32",
    "Nexus_10_API_32",
    "Pixel_5_API_32",
...
async def start_android_emulators(android_emulator_names):
    ...
    # These need the flutter lock, so just run these in order.
    for name in android_emulator_names:
        await start_android_emulator(name)
...
async def main():
    ...
    ios_dir = f"ios/{args.locale}"
    android_dir = f"android/{args.locale}"

    if args.clear_screenshots:
        await run_command(["rm", "-rf", ios_dir])
        await run_command(["rm", "-rf", android_dir])
