// Taken from Magnum6:
// https://github.com/magnum6actual/Aptos-Tutorial/blob/ea9e83da1486688137ce5b9a589272578de73f5a/sources/MapTable.move
...
module maptable::MapTable {
    ...
    struct MapValue<K: copy + store + drop, V: store> has store {
        val: V,
        prev: Option<K>,
        next: Option<K>,
    }
    ...
    struct MapTable<K: copy + store + drop, V: store> has store {
        inner: Table<K, MapValue<K, V>>,
        head: Option<K>,
        tail: Option<K>,
		keys: vector<K>
    }
    ...
    public fun new<K: copy + store + drop, V: store>(): MapTable<K, V> {
        MapTable {
            inner: Table::new(),
            head: Option::none(),
            tail: Option::none(),
    ...
    public fun destroy_empty<K: copy + store + drop, V: store>(table: MapTable<K, V>) {
        assert!(empty(&table), 0);
        assert!(Option::is_none(&table.head), 0);
        assert!(Option::is_none(&table.tail), 0);
        let MapTable {inner, head: _, tail: _, keys: _} = table;
    ...
    public fun length<K: copy + store + drop, V: store>(table: &MapTable<K, V>): u64 {
        Table::length(&table.inner)
    }

    /// Returns true if this table is empty.
    public fun empty<K: copy + store + drop, V: store>(table: &MapTable<K, V>): bool {
    ...
    public fun tail_key<K: copy + store + drop, V: store>(table: &MapTable<K, V>): Option<K> {
        table.tail
    }
